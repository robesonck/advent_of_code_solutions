...
const originalPatterns = [
  { numberLetters: ["a", "b", "c", "e", "f", "g"], number: 0 }, // 0
  { numberLetters: ["c", "f"], number: 1 }, // 1
  { numberLetters: ["a", "c", "d", "e", "g"], number: 2 }, // 2
  { numberLetters: ["a", "c", "d", "f", "g"], number: 3 }, // 3
  { numberLetters: ["b", "c", "d", "f"], number: 4 }, // 4
  { numberLetters: ["a", "b", "d", "f", "g"], number: 5 }, // 5
  { numberLetters: ["a", "b", "d", "e", "f", "g"], number: 6 }, // 6
  { numberLetters: ["a", "c", "f"], number: 7 }, // 7
  { numberLetters: ["a", "b", "c", "d", "e", "f", "g"], number: 8 }, // 8
  { numberLetters: ["a", "b", "c", "d", "f", "g"], number: 9 }, // 9
];

export const countUnique = (patterns: Pattern[]) => {
  let count = 0;
  patterns.forEach(({ outputs }) => {
    outputs.forEach((output) => {
      Object.entries(uniqueNumbersByLeds).forEach(([ledCount]) => {
        if (output.length === Number(ledCount)) {
          count++;
        }
      });
    });
...
export const sumDecoded = (patterns: Pattern[]) => {
  let sum = 0;
  patterns.forEach(({ signalPatterns, outputs }) => {
    let remainingSignalPatterns = [...signalPatterns];
    let newLetters: string[] = new Array(10);

    const newAlphabetPossibilities = {
      a: ["a", "b", "c", "d", "e", "f", "g"],
      b: ["a", "b", "c", "d", "e", "f", "g"],
      c: ["a", "b", "c", "d", "e", "f", "g"],
      d: ["a", "b", "c", "d", "e", "f", "g"],
    ...
    const removePossibilities = (from: string[], to: string[]) => {
      from.forEach((elFrom) => {
        newAlphabetPossibilities[elFrom] = newAlphabetPossibilities[
          elFrom
        ].filter((elTo) => to.includes(elTo));
      });
    };

    const addNumberFound = (num: number, pattern: string) => {
      newLetters[num] = pattern;
      remainingSignalPatterns = remainingSignalPatterns.filter(
        (p) => p !== pattern
      );
      removePossibilities(
        pattern.split(""),
        originalPatterns[num].numberLetters
      );
    };

    signalPatterns.forEach((pattern) => {
      Object.entries(uniqueNumbersByLeds).forEach(
        ([ledCount, currentNumber]) => {
          if (pattern.length === Number(ledCount)) {
            addNumberFound(currentNumber, pattern);
          }
        }
      );
    });

    // Play Sudoku with repetitions of 2 and 1
    const sudokuElimination = () => {
      let checkLength = 2;
      while (checkLength > 0) {
        const doublePossibilities = Object.values(
          newAlphabetPossibilities
        ).reduce((acc: Record<string, number>, possibility) => {
          if (possibility.length === checkLength) {
            const key = String(possibility);
            if (!acc[key]) {
              acc[key] = 0;
            }
            acc[key] += 1;
        ...
        let onePassed = false;
        Object.entries(doublePossibilities).forEach(([pair, quantity]) => {
          if (quantity === checkLength) {
            const currentPair: string[] = pair.split(",");
            for (const key in newAlphabetPossibilities) {
              if (newAlphabetPossibilities[key].length !== checkLength) {
                newAlphabetPossibilities[key] = newAlphabetPossibilities[
                  key
                ].filter((v) => {
                  const includes = currentPair.includes(v);
                  if (includes) {
                    onePassed = true;
                  }
                  return !includes;
        ...
        if (onePassed || checkLength === 2) {
          checkLength = checkLength === 2 ? 1 : 2;
        } else {
          checkLength = 0;
        }
      }
    ...
    // For number 6, it has to have both of the number 1 and the length of it's segments (only 0 and 9 with that). With that we could guess the missing segment and therefore assign it to a letter. Probably we could do something similar with the others
    const oneLetters = newLetters[1];
    remainingSignalPatterns.filter((pattern) => {
      const has6letters = pattern.length === 6;
      if (has6letters) {
        const hasOneLetters = oneLetters.split("").every((l) => {
          return pattern.includes(l);
        });
        if (!hasOneLetters) {
          addNumberFound(6, pattern);
        }
      }
    });
    ...
    // Check remaining signals for possible matches to add
    const bruteForceCheck = () => {
      let foundMatch = false;
      remainingSignalPatterns.forEach((pattern) => {
        let possibleOriginalPatterns = originalPatterns.filter(
          ({ numberLetters, number }) => {
            const hasSameLength = numberLetters.length === pattern.length;
            const isRemaining = !Boolean(newLetters[number]);
            return hasSameLength && isRemaining;
          }
        );

        for (let i = 0; i < pattern.length; i++) {
          const letter = pattern[i];
          const possibleLetters: string[] = newAlphabetPossibilities[letter];
          possibleOriginalPatterns = possibleOriginalPatterns.filter((match) =>
            possibleLetters.some((l) => match.numberLetters.includes(l))
          );
        }

        if (possibleOriginalPatterns.length === 1) {
          foundMatch = true;
          addNumberFound(possibleOriginalPatterns[0].number, pattern);
        }
      });
      return foundMatch;
    };

    let shouldContinueBruteForce = true;
    while (shouldContinueBruteForce && remainingSignalPatterns.length > 0) {
      shouldContinueBruteForce = bruteForceCheck();
    }

    // Sum with outputs using newLetters
    const outputNumberStr = outputs.reduce((acc, output) => {
      for (let num = 0; num < newLetters.length; num++) {
        const letter = newLetters[num];
        if (isEqual(letter.split("").sort(), output.split("").sort())) {
          return acc + num;
        }
      }
      return acc;
    }, "");
    sum += Number(outputNumberStr);
  });
  return sum;
};
...
const processInput = (input: string) => {
  return input.split("\n").map((line) => {
    ...
    return { signalPatterns: left.split(" "), outputs: right.split(" ") };
  });
};

const preInput = `acedgfb cdfbe gcdfa fbcad dab cefabd cdfgeb eafb cagedb ab | cdfeb fcadb cdfeb cdbaf`;

const baseInput = `be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb | fdgacbe cefdb cefbgd gcbe
...
const realProblem = `febca cfagb ecbafd efdcbg cbegdfa fg bgafec gfae acgdb gfc | cgf facdeb ecgfdb afcbge
...
describe("Seven Segment Search", () => {
  describe("countUnique", () => {
    it("Solves base problem", () => {
      ...
      const result = countUnique(input);
  ...
  describe("sumDecoded", () => {
    it("Solves minimal problem", () => {
      const input = processInput(preInput);
      const result = sumDecoded(input);
